...
public class Player {
    ...
    public void drawHand(SpriteBatch batch) {
        int offset = 0;
        int cardSize = Values.CARD_SIZE_X / 2;

        if (hand.size() > 0) {


            for (Card c : hand) {
                offset += Values.HAND_OFFSET;
                c.setxPos(Values.FRIENDLY_HAND_X + offset - (hand.size() * cardSize));
                c.setyPos(Values.FRIENDLY_HAND_Y);
    ...
    public void drawEnemyHand(SpriteBatch batch) {
        ...
        if (hand.size() > 0) {
            ...
            if (hand.size() == 1) {
                Card c = hand.get(0);
                c.setxPos(Values.ENEMY_HAND_X - c.getCardArt().getWidth());
                c.setyPos(Values.ENEMY_HAND_Y);
                batch.draw(hand.get(0).getCardArt(), c.getxPos(), c.getyPos());
            } else {
                for (Card c : hand) {
                    offset += Values.HAND_OFFSET;
                    c.setxPos(Values.ENEMY_HAND_X + offset - (hand.size() * cardSize));
                    c.setyPos(Values.ENEMY_HAND_Y);
                    batch.draw(PlayScreen.cardBg, c.getxPos(), c.getyPos(), Values.CARD_SIZE_X, Values.CARD_SIZE_Y);
